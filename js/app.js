...
const els = {
  ...
  membersModalMeta: document.getElementById("membersModalMeta"),
...
function updateChatAreaState() {
  ...
  const isGroup = state.currentConversation?.kind === "group";
  els.chatArea.classList.toggle("idle", !hasConversation);
  els.composer.classList.toggle("hidden", !hasConversation);
  els.chatHeaderActions.classList.toggle("hidden", !hasConversation);
  els.leaveGroupBtn.classList.toggle("hidden", !isGroup);
  els.manageMembersBtn.classList.toggle("hidden", !isGroup);
}
...
function renderMembersModal() {
  ...
  if (!state.groupMembers.length) {
    els.membersList.innerHTML = `<div class="request-card"><div class="request-meta">Nenhum membro encontrado.</div></div>`;
    return;
  }

  state.groupMembers.forEach((member) => {
    const row = document.createElement("div");
    const isSelf = member.username === loggedUser;
    row.className = "member-row";
    row.innerHTML = `
      <div class="avatar small">${escapeHtml(initials(member.display_name || member.username))}</div>
      <div class="member-meta">
        <strong>${escapeHtml(member.display_name || member.username)}</strong>
        <small>@${escapeHtml(member.username)}${member.role === "owner" ? " • dono" : ""}</small>
      </div>
    ...
    if (state.groupMeta?.can_manage_members && !isSelf) {
      const removeBtn = document.createElement('button');
      removeBtn.className = 'ghost-btn small';
      removeBtn.type = 'button';
      removeBtn.textContent = 'Remover';
      removeBtn.addEventListener('click', () => removeGroupMember(member.username));
...
async function loadGroupMembers() {
  if (!state.currentConversationId || state.currentConversation?.kind !== 'group') return;
  setMembersError('');
  const data = await apiGet('groups/members', {
    username: loggedUser,
    conversation_id: state.currentConversationId
  });
  state.groupMembers = data.members || [];
  state.groupMeta = data.group || null;
  els.membersModalMeta.textContent = state.groupMeta?.can_manage_members
    ? 'Você pode remover membros deste grupo.'
    : 'Você pode ver os membros e sair do grupo.';
  renderMembersModal();
}
...
async function openMembersModal() {
  ...
  els.membersList.innerHTML = `<div class="request-card"><div class="request-meta">Carregando membros...</div></div>`;
  try {
    await loadGroupMembers();
  } catch (error) {
    setMembersError(error.message || 'Não foi possível carregar os membros.');
  }
}

async function leaveCurrentGroup() {
  if (!state.currentConversationId || state.currentConversation?.kind !== 'group') return;
  const ok = window.confirm(`Sair de "${state.currentConversation.title}"?`);
  if (!ok) return;
  try {
    ...
    const title = state.currentConversation.title;
    state.currentConversationId = null;
    state.currentConversation = null;
    state.messages = [];
    closeMembersModal();
    ...
    renderMessages();
    updateChatHeader();
    showToast(`Você saiu de "${title}".`, 'success');
  } catch (error) {
    alert(error.message);
  }
}

async function removeGroupMember(targetUsername) {
  if (!state.currentConversationId) return;
  const ok = window.confirm(`Remover @${targetUsername} deste grupo?`);
  if (!ok) return;
  try {
    await apiPost('groups/remove-member', {
      username: loggedUser,
      conversation_id: state.currentConversationId,
      target_username: targetUsername
    });
    showToast(`@${targetUsername} removido do grupo.`, 'success');
    await loadGroupMembers();
    await refreshSidebar();
  } catch (error) {
    setMembersError(error.message || 'Não foi possível remover esse membro.');
  }
}
...
document.querySelectorAll("[data-close-members-modal]").forEach((element) => {
  element.addEventListener("click", closeMembersModal);
});

els.manageMembersBtn.addEventListener('click', openMembersModal);
els.leaveGroupBtn.addEventListener('click', leaveCurrentGroup);
