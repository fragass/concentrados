...
function renderRequestLists() {
  ...
  if (!state.incomingRequests.length) {
    els.incomingRequests.innerHTML = `<div class="request-card"><div class="request-meta">Nenhum convite recebido.</div></div>`;
  } else {
  ...
  if (!state.outgoingRequests.length) {
    els.outgoingRequests.innerHTML = `<div class="request-card"><div class="request-meta">Nenhum convite enviado pendente.</div></div>`;
  } else {
...
function renderConversationList() {
  ...
  if (!state.conversations.length) {
    els.conversationList.innerHTML = `<div class="conversation-card"><div class="request-meta">Nenhuma conversa ainda.</div></div>`;
    return;
  }
...
function updateChatHeader() {
  if (!state.currentConversation) {
    ...
    els.chatSubtitle.textContent = "Seus contatos e grupos aparecem na lateral.";
    return;
  }
...
els.userSearchInput.addEventListener("input", () => {
  ...
  state.searchTimer = window.setTimeout(searchUsers, 300);
});
