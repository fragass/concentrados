    ...
    if (user.relation === "contact") {
      ...
      button.addEventListener("click", async () => {
        try {
          ...
          state.searchResults = [];
          renderSearchResults();
          setSearchFeedback(`Convite enviado para @${user.username}.`);
...
function renderRequestLists() {
  ...
  if (!state.incomingRequests.length) {
    els.incomingRequests.innerHTML = `<div class="request-card empty-row"><div class="request-meta">Nenhum convite recebido.</div></div>`;
  } else {
  ...
  if (!state.outgoingRequests.length) {
    els.outgoingRequests.innerHTML = `<div class="request-card empty-row"><div class="request-meta">Nenhum convite enviado.</div></div>`;
  } else {
...
function renderConversationList() {
  ...
  if (!state.conversations.length) {
    els.conversationList.innerHTML = `<div class="conversation-card empty-row"><div class="request-meta">Nenhuma conversa ainda.</div></div>`;
    return;
  }
...
function updateChatHeader() {
  ...
  if (!state.currentConversation) {
    ...
    els.chatSubtitle.textContent = "O chat só aparece quando você abrir uma conversa.";
    return;
  }
...
async function searchUsers() {
  ...
  if (!query) {
    state.searchResults = [];
    renderSearchResults();
    setSearchFeedback("Digite um username para buscar.");
    return;
  }
  ...
  try {
    ...
    if (state.searchResults.length) {
      setSearchFeedback(`${state.searchResults.length} resultado(s) encontrado(s).`);
    } else {
      setSearchFeedback("Nenhum usuário encontrado.");
    }
  } catch (error) {
    state.searchResults = [];
    renderSearchResults();
