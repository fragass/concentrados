...
async function getGroupContext(conversationId, username) {
  const { data: membership, error: membershipError } = await supabaseService
    .from("conversation_members")
    .select(`
      role,
      conversations ( id, kind, title, created_by )
    `)
    .eq("conversation_id", conversationId)
    .eq("username", username)
    .maybeSingle();
  ...
  if (!membership || !membership.conversations) {
    const err = new Error("Você não participa desse grupo");
    err.statusCode = 403;
    throw err;
  }
  if (membership.conversations.kind !== 'group') {
    const err = new Error("Essa conversa não é um grupo");
    err.statusCode = 400;
    throw err;
  }
  return {
    conversation: membership.conversations,
...
async function handleGroupMembersGet(req, res) {
  if (req.method !== "GET") return sendJson(res, 405, { success: false, message: "Método não permitido" });
  ...
  try {
    ...
    const context = await getGroupContext(conversationId, username);
    ...
    if (error) throw new Error(error.message);
    const profileMap = await getProfileMap((members || []).map((item) => item.username));

    return sendJson(res, 200, {
      success: true,
      group: {
        id: context.conversation.id,
        title: context.conversation.title || 'Grupo sem nome',
...
async function handleGroupLeave(req, res) {
  if (req.method !== 'POST') return sendJson(res, 405, { success: false, message: 'Método não permitido' });
  ...
  try {
    ...
    const remaining = (members || []).filter((item) => item.username !== username);
    ...
    if (!remaining.length) {
      const { error: convDeleteError } = await supabaseService
        .from('conversations')
        .delete()
        .eq('id', conversationId);
      if (convDeleteError) throw new Error(convDeleteError.message);
      return sendJson(res, 200, { success: true, deleted_group: true });
    }
    ...
    if (context.role === 'owner') {
      ...
      const { error: ownerError } = await supabaseService
        ...
        .update({ role: 'owner' })
        .eq('conversation_id', conversationId)
        .eq('username', nextOwner.username);
...
async function handleGroupRemoveMember(req, res) {
  if (req.method !== 'POST') return sendJson(res, 405, { success: false, message: 'Método não permitido' });

  try {
    const body = await readJsonBody(req);
    const username = safeUsername(body.username);
    const targetUsername = safeUsername(body.target_username);
    const conversationId = Number(body.conversation_id);
    if (!username || !targetUsername || !conversationId) {
      return sendJson(res, 400, { success: false, message: 'Dados inválidos' });
    }

    if (username === targetUsername) {
      return sendJson(res, 400, { success: false, message: 'Use a opção de sair do grupo para remover a si mesmo.' });
    }

    const context = await getGroupContext(conversationId, username);
    if (!context.canManage) {
      return sendJson(res, 403, { success: false, message: 'Só o dono do grupo pode remover membros.' });
    }
    ...
    if (!targetMember) {
      return sendJson(res, 404, { success: false, message: 'Membro não encontrado.' });
    }
    if (targetMember.role === 'owner') {
      return sendJson(res, 400, { success: false, message: 'O dono do grupo não pode ser removido por aqui.' });
    }
...
async function handler(req, res, routeOverride) {
  ...
  try {
    ...
    if (route === "groups/members") return handleGroupMembersGet(req, res);
    if (route === "groups/leave") return handleGroupLeave(req, res);
    if (route === "groups/remove-member") return handleGroupRemoveMember(req, res);
